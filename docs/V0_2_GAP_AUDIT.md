...
- Settings schema v5 migration. Existing model, Gateway key, custom endpoint, and app permissions remain intact; named subscription configuration is added without resetting them.
...
- Named local subscription runtime boundary for Codex, Claude Code, and OpenCode: read-only executable/auth probing, optional model IDs, ephemeral task launch, provider-specific MCP wiring, bounded token extraction, and shared OpenUse tool/policy enforcement.
...
- Codex `0.154.0` and Claude Code `2.1.202` were found and authenticated on the host. The Codex named-runtime MCP smoke test completed through two guarded tool calls and returned structured token usage. A real Claude task was not completed because the authenticated CLI remained in API retry; OpenCode was not runnable from the installed Windows package on this host.
