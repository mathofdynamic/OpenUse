...
- Electron 43 exposes BrowserWindow transparency, `vibrancy`, and `visualEffectState`. OpenUse uses macOS under-window vibrancy and Windows `setBackgroundMaterial("acrylic")` on a non-layered window, with a native rounded shape to keep the DWM backdrop inside the visible boundary. The renderer falls back to a neutral material when the Windows API is unavailable. See the [BrowserWindow](https://www.electronjs.org/docs/latest/api/browser-window) and [BaseWindow](https://www.electronjs.org/docs/latest/api/base-window) references.
